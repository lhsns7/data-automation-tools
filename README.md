...
| [`tools/biz_app`](tools/biz_app) | 내부 업무 웹앱 골격 — 로그인·역할 권한·CSRF, 스키마 선언식 CRUD(검색·필터·페이징), 엑셀 가져오기(불량 격리)/내보내기, **감사 로그(전→후)**, **동시 편집 보호(409)**, 참조 무결성. 표준 라이브러리+SQLite | 세 역할 세션의 실제 HTTP 공격으로 8항목: viewer 등록·staff 삭제 **403·저장 0** · CSRF 403 · 동시 편집 **둘째 409·첫 값 보존** · 엑셀 왕복 전수 · 감사 로그 수=동작 수 |
| [`tools/llm_app`](tools/llm_app) | 문서 기반 AI 챗봇 서비스 골격 — 업로드→인덱스, 근거 인용·정직 거절, **비용 게이트(일일/월 상한·견적·캐시)**, 사용량 회계, 대화 격리, 평가셋 리포트·거절 임계 캘리브레이션. 생성형은 API 게이트 뒤(데모 = 모의) | 서버 실구동 9항목: 홀드아웃 **오답 0**·5/5 · 일일 상한 3 → **4번째 거절·호출 정확히 3**·캐시 호출 0·월 상한 거절 · 회계 합 = 백엔드 카운터 · 타인 대화 403 |
| [`tools/codebase_audit`](tools/codebase_audit) | 레거시 코드 인수 진단 킷 — 소스 트리 → 인벤토리·의존성(미고정 버전)·**위험 신호 좌표**(하드코딩 비밀·SQL 결합·eval·인코딩 혼재·중복)·라우트/DB/테스트 추정·리스크 점수·1주차 착수 순서. 언어 무관·오프라인 | 결함을 심은 합성 트리(PHP+Java+JS+SQL): 심은 위험 10종 **파일:행 전수·초과 0** · vendor 제외 · 클린 트리 **오탐 0** · 의존성 미고정 3/3 · 라우트·테이블·테스트 카운트 정확 |
